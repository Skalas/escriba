...
### Fixed
- **Stop no longer calls a slow drain a failure.** `stop()` gave the audio-processing thread a flat 10s to finish its final flush — which *transcribes* the leftover buffer, so on a backlogged transcriber it legitimately runs longer (observed: the thread finished 0.3s after the timeout fired). The session was marked `error` and the final flush + WAV close were skipped. The budget is now derived from the work that can actually be pending: `max(60s, chunk_duration × 6)`, since the buffer is capped at 2 chunks and a loaded machine transcribes at about half of realtime.
- **A stop-time error is no longer permanent.** `/api/status` re-served `session.error` for the life of the app and the dashboard poller re-raised the banner every 2s with no path that ever hid it. The error is now delivered once (`consume_error()`), the banner is click/keyboard-dismissible with a 15s auto-clear, and re-transcribing an errored session clears its row back to `completed` (a rebuilt transcript supersedes the stop failure) along with the pending in-memory error.
- **Local AI notes failed silently.** A thinking-capable local model (gemma-4) could spend its entire token budget in the reasoning channel; `_extract_response` collapsed that to `""` and every layer above returned a bare "Failed to generate notes" with *nothing* in the log. Long-form notes and summaries now run with thinking disabled, an unusable result is logged with model, budget, and whether it truncated mid-reasoning, and the endpoint names the model in its error.
...
- **Mic-only capture no longer drops audio when system audio is partially buffered** after the `ChunkPump` extraction. Closes the behavior-preserving seam of #103 / #134.
