...
from escriba.audio.call_detection import (
    wait_for_call_start,
)
...
from escriba.audio.live_capture import run_live_capture, run_streaming_capture
from escriba.config import AppConfig, config_to_dict
from escriba.daemon.client import DaemonClient, is_daemon_running
from escriba.daemon.server import run_daemon
from escriba.watch.watch_folder import watch_folder
...
app = typer.Typer(
    ...
    help="Escriba - Local system + mic audio transcription (macOS).",
)
...
def _main_callback(
    ...
    config: Path | None = typer.Option(
        None,
        "--config",
        help="Path to Escriba TOML config.",
        exists=False,
        dir_okay=False,
        file_okay=True,
        readable=True,
...
def cmd_app(ctx: typer.Context) -> None:
    ...
    cfg = _get_cfg(ctx)
    from escriba.app.menubar import run_menubar_app
...
def cmd_create_issues(
    ...
    from escriba.integrations.github import create_issues_from_transcript
