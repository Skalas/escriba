...
from escriba.audio.device_detection import auto_detect_devices
...
from escriba.watch.watch_folder import watch_folder, wait_for_queue_empty
...
def run_live_capture(
    ...
    *,
    ...
    temp_segment_dir = Path(tempfile.mkdtemp(prefix="escriba-temp-"))
    watched_segment_dir = Path(tempfile.mkdtemp(prefix="escriba-watched-"))
        ...
        if notify_enabled and summary_output and summary_output.exists():
            try:
                if notify_platform == "telegram":
                    from escriba.notify.telegram import send_summary
